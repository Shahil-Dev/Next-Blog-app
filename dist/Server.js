...
var require_prisma = __commonJS({
  "src/generated/prisma/index.js"(exports2) {
    ...
    Prisma2.PrismaClientValidationError = PrismaClientValidationError3;
    ...
    exports2.Prisma.FriendRequestScalarFieldEnum = {
      id: "id",
      senderId: "senderId",
    ...
    exports2.Prisma.ReactionScalarFieldEnum = {
      id: "id",
      type: "type",
      userId: "userId",
      postId: "postId",
    ...
    exports2.RequestStatus = exports2.$Enums.RequestStatus = {
      PENDING: "PENDING",
      ACCEPTED: "ACCEPTED",
      REJECTED: "REJECTED"
    };
    exports2.ReactionType = exports2.$Enums.ReactionType = {
      LOVE: "LOVE",
      HAHA: "HAHA",
      SAD: "SAD",
      ANGRY: "ANGRY"
    };
    exports2.CommentStatus = exports2.$Enums.CommentStatus = {
      APPROVED: "APPROVED",
      FLAGGED: "FLAGGED",
      REJECTED: "REJECTED"
    };
...
var require_client = __commonJS({
  "src/generated/prisma/client.js"(exports2, module2) {
    "use strict";
    ...
    sendVerificationEmail: async ({ user, url, token }) => {
      ...
      try {
        const info = await transporter.sendMail({
          ...
          text: `Hi ${user.name}, please verify your email by visiting this link: ${verificationURL}`,
          // Plain text fallback-এও
          html: emailHtml
        });
  ...
  const result = await prisma.post.findUnique({
    ...
    include: {
      comments: {
        where: {
          parentId: null,
          status: import_client.CommentStatus.APPROVED
        },
        orderBy: { createdAt: "desc" },
        include: {
          replies: {
            where: { status: import_client.CommentStatus.APPROVED }
          }
        }
      },
      _count: { select: { comments: true } }
...
var updatePost = async (postId, data, authorId) => {
...
var deletedPost = async (postId, userId, isAdmin) => {
  const postData = await prisma.post.findUnique({
    where: { id: postId }
  });
  if (!postData) {
...
var getState = async () => {
  return await prisma.$transaction(async (tx) => {
    const [
      ...
      tx.post.count(),
      tx.comment.count(),
      tx.comment.count({ where: { status: import_client.CommentStatus.APPROVED } }),
      tx.comment.count({ where: { status: import_client.CommentStatus.REJECTED } }),
      tx.user.count(),
...
var PaginationAndSortingHelper = (option) => {
...
var getAllPosts2 = async (req, res) => {
  try {
    ...
    const { page, limit, skip: skip2, SortBy, SortOrder } = PaginationAndSortingHelper_default(
      req.query
    );
...
var getPostById = async (req, res) => {
  try {
    ...
    const result = await PostService.geAllPostByID(id);
...
var deletedPost2 = async (req, res) => {
  try {
    const user = req.user;
    const { postId } = req.params;
    if (!user) {
...
router.get(
  "/getStats",
  authMiddleware("ADMIN" /* ADMIN */, "SUPER_ADMIN" /* SUPER_ADMIN */),
...
router.patch(
  "/:postId",
  authMiddleware("USER" /* USER */, "ADMIN" /* ADMIN */),
...
var getCommentById = async (commentId) => {
...
var deletedComment = async (commentId, userId) => {
...
var updateComment = async (commentId, data, userId) => {
  const commentData = await prisma.comment.findUnique({
    where: { id: commentId },
    select: { id: true, userId: true }
  });
...
var moderatedComment = async (id, data) => {
  const commentData = await prisma.comment.findUnique({
    where: { id },
...
var deletedComment2 = async (req, res) => {
  try {
    const user = req.user;
    const { commentId } = req.params;
    if (!user || !user.id) {
...
var moderatedComment2 = async (req, res) => {
  try {
    const { commentId } = req.params;
    if (!commentId) {
      return res.status(400).json({ success: false, message: "Comment ID is required" });
...
var CommentController = {
  ...
  deletedComment: deletedComment2,
  updateComment: updateComment2,
  moderatedComment: moderatedComment2
};
...
// src/modules/Reaction/reaction.route.ts
var import_express3 = __toESM(require("express"));

// src/modules/Reaction/reaction.service.ts
var createReact = async (payload) => {
...
var ReactionRoutes = router3;

// src/modules/FriendRequest/friendRequest.route.ts
var import_express4 = __toESM(require("express"));

// src/modules/FriendRequest/friendRequest.service.ts
var sendFriendRequest = async (senderId, receiverId) => {
...
var acceptFriendRequest = async (userId, requestId) => {
...
var rejectFriendRequest = async (userId, requestId) => {
  const request = await prisma.friendRequest.findUnique({
    where: { id: requestId }
  });
  if (!request) throw new Error("Friend request not found");
...
var cancelFriendRequest = async (userId, requestId) => {
  const request = await prisma.friendRequest.findUnique({
    where: { id: requestId }
  });
  if (!request) throw new Error("Friend request not found");
...
var unfriendUser = async (userId, targetUserId) => {
...
var isFriend = async (userId1, userId2) => {
...
// src/modules/FriendRequest/friendRequest.controller.ts
var sendRequest = async (req, res) => {
  try {
...
router4.use(authMiddleware("USER" /* USER */, "ADMIN" /* ADMIN */));
router4.post("/send", FriendRequestController.sendRequest);
router4.patch("/accept/:requestId", FriendRequestController.acceptRequest);
router4.patch("/reject/:requestId", FriendRequestController.rejectRequest);
router4.delete("/cancel/:requestId", FriendRequestController.cancelRequest);
router4.delete("/unfriend", FriendRequestController.unfriend);
var FriendRequestRoutes = router4;
...
var import_library = require("@prisma/client/runtime/library.js");
function globalErrorHandler(err, req, res, next) {
  let statusCode = 500;
  let errorMessage = "Internal server Error";
  let errorDetails = err;
  if (err instanceof import_library.PrismaClientValidationError) {
    statusCode = 400;
